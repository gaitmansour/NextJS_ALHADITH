    ...
    return (
        <TemplateArticle ListBreadcrumb={data} titlePage="سؤال و جواب">
            <Body className="TemplateArticleBody Media d-flex p-4">
                <div className="flex-fill">
                    {DataQuestion && DataQuestion.length > 0 ?
                        <div>
                            ...
                            <h3 className={"h3"}>الإجابة</h3>
                            {DataQuestion && DataQuestion.map((item, index) => {
                                // console.log("-------------||||||",item)
                                return (
                                    <div key={index} className="card Card-answer1 w-100 my-5"
                        ...
                        <div>
                            ...
                            <div className="card Card-answer1 w-100 my-5">
                                <div className="card-body ">
                                    <p className="p card-text">
                ...
                <div className="side-bar w-25 px-4">
                    <p className="p tiitle">مواد ذات صلة</p>
                    <div className="SimpleList">
                        {DataQuestions1?.data.filter((item) => item.relationships.taxonomy_forums.data.id === categoryId).map((item, i) => {
                            return (
                                <Link
                                    key={i}
                                    exact
                                    ...
                                    href={{
                                        ...
                                        query: {
                                            ...
                                            itemQuestion: item?.attributes?.body?.processed.replace(/<[^>]+>/g, ''),
