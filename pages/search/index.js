    ...
    const handleTopicFrom = async (res, r, router, narList, sourceList) => {
        ...
        if (from === 'section') {
            ...
            const ArrayCategory =
                res && res.length > 0 && res?.filter((item) => item.label === topic)
            const ArrayNars =
                narList && narList.length > 0 && narList?.filter((item) => item.label === narrator)
            const ArraySource =
                sourceList && sourceList.length > 0 && sourceList?.filter((item) => item.label === source)
            const ArrayDegree =
                r && r.length > 0 && r?.filter((item) => item.label === content)
            setChoiceCategory(ArrayCategory[0])
            setChoiceDegree(ArrayDegree[0])
            ...
            handleSearch(word, ArrayCategory[0], ArrayDegree[0], ArraySource[0], ArrayNars[0])
        }
    }
