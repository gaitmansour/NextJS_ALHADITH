...
export const getResourcesData = (langcode, value="موارد") => `${api_url}/section?fields[node--section]=title,body,field_icone,field_lien&filter[langcode]=${langcode}&filter[alqsm][condition][path]=field_alqsm.name&filter[alqsm][condition][value]=${value}&include=field_icone`
...
export const getMenuByName = (name)=>`${base_url}/en/menus?name=${name}&parent_target_id=""`
export const getMenuLinks = (show=1)=>`${base_url}/en/menus?show=${show}`
export const getArticleByIdd = (title) => `${api_url}/article?include=field_image&fields[node--article]=title,body,field_image,field_titre&filter[field_titre]=${title}`
...
export const getQuestionbyID=(nID)=>`${base_url}/en/commentaire?entity_id=${nID}`
export const SubmitQuestionAPI=()=>`${api_url}/question`
export const getQuestionAPI=()=>`${api_url}/forum`
export const AllForums=()=>`${api_url}/forum?fields[node--forum]=title,created,body,comment_forum,taxonomy_forums,drupal_internal__nid`
export const getTaxonomyForum=()=>`${base_url}/en/taxonomy/forums`
export const getSideItems=(id)=>`${base_url}/en/menu_by_parent?parent_target_id=${id}`
export const newsletter =()=>`${api_newsletter}`
export const getLive = () => `${base_url}/live`
export const getCarousel = () => `${api_url}/article?fields[node--article]=title,body&include=field_image&filter[field_est_slider]=1`
