      ...
      MenuLinks?.map((item, index) => {
        ...
        const MenuDropDown = React.forwardRef(({ children, style, className, "aria-labelledby": labeledBy }, ref) => {
          ...
          if (lengthListItems > 0) {
            return (
              <ul ref={ref} style={{ ...style, transform: "none", top: "100%" }} className={`${className} shadow-card ${styles.contentDropdown} ${styleAlignText}`} aria-labelledby={labeledBy}>
                {item?.items?.map((data, i) => {
                  return (
                    <Link
                      ...
                      <a className={`${styles.navBarActive} ${styles.listMenu}`}>
                        <li
                          className={`btn justify-content-start rounded-0 ${styles.p3 ? styles.p3 : ""} ${lengthListItems == ++i ? "border-0" : ""}`}
                          onClick={() => {
        ...
        return (
          <Dropdown className={resizeHeightIsScrollable} key={index.toString()} onToggle={(isOpen, metadata) => onToggleHandler(isOpen, metadata)} autoClose="outside">
    ...
    const menuLinks =
      ...
      MenuGlobal?.map((item, index) => {
        if (item?.label !== "الرئيسية") {
          return (
            <div className="col-md-2 nav-item flex-column pt-4" key={index.toString()} ref={ref}>
              <Link
                ...
                href="#">
                <a className={`nav-link d-flex align-items-center text-dark p-0 fw-bold title-link`}>{item?.label}</a>
              </Link>
              {item?.items?.length > 0 && (
  ...
  return (
    <div className="NavBar bg-white navbar navbar-expand-lg sticky-top navbar-light p-0">
      <div className="container-fluid p-0">
        <div className={`${visible ? styles.menuBlog : styles.menuAl} btn rounded-0 p-0 border-0 align-self-stretch d-flex align-items-center justify-content-center menu-btn bg-gradient-green`} style={props?.styleBtnBars} onClick={() => setShowMenu(!showMenu)}>
        ...
        {!showLogo && (
          <>
            <div className={`${visible ? "d-block" : "d-none"} d-flex align-items-center btn m-0 p-0 ${styles.logoSticky}`}>
              <Link className="my-2" href="/">
                <Image className="logoNav" src={Logos.logo_portail_hadith} alt="logo-Al-hadith-Mohammed-VI" title="logo Al hadith Mohammed VI" />
              </Link>
            </div>
          </>
        )}
        ...
        <div className={`bg-white global-menu position-absolute px-4 ${showMenu ? "global-menu-show" : "global-menu-hide"} overflow-hidden`}>
          <i className="fas fa-times-circle text-white icon-close" onClick={() => setShowMenu(!showMenu)} />
          <div
            className="row"
        ...
        <div className={`${visible ? "d-block" : "d-none"}`}>
          {InputShow ? (
            <div className={styles.responsiveSearch} style={{ width: "60vw" }}>
              <SearchInput styleIcon={{ color: "#157646", width: 20 }} styleFilter={{ backgroundColor: "#157646", width: 50 }} styleSerachIcon={{ backgroundColor: "#157646" }} {...props} input={input} onChange={(v) => handleInput(v)} clickSearch={() => handleClickSearch()} placeholder="البحث في منصة الحديث النبوي الشريف" className={`${styles.searchSection} text-black bg-white mx-0 shadow-card`} />
            </div>
          ) : (
            <Link href={"/search"}>
              <a onClick={props.onClickSettings} className={` d-flex align-items-center btn m-0 p-0 searchSticky`}>
                <i className="fas fa-search p-3 fa-2x iconSearch" style={{ color: "#ADABAB" }} />
              </a>
            </Link>
          )}
        </div>
        <div
          ...
          <Link exact activeClassName="active" href="/QuestionsReponses" passHref={true} as={"/QuestionsReponses"}>
            <a href={"/dkfjhskj"} className={` align-items-center d-flex px-4 py-2 ${styles.linkQa}`}>
              <Image className={`logo mx-0 px-1`} src={Icons.icon_faq} alt="icon faq" />
