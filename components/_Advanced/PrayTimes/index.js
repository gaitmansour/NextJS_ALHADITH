...
const PrayTimes = () => {
  ...
  const fetchAPI = async () => {
    ...
    const url_get_times = `${api_search}/api/prieres/ville/${currentCity}/${
      new Date().getMonth() + 1
    }/${new Date().getDate()}`
    ...
    axios
      ...
      .then(
        axios.spread((data_date_hijri, data_get_cities, data_get_times) => {
          ...
          if (
            data_date_hijri.status === 200 &&
            data_date_hijri.data !== undefined
          ) {
    ...
    <Cards className={styles.praytimes}>
      <div className='d-flex justify-content-between align-items-center mb-4'>
        <div className='d-flex flex-column align-self-stretch justify-content-between'>
          <h3 className={`${styles.title} title text-success m-0`}>
            {'مواقيت الصلاة'}
          </h3>
