...
const SliderVideoList = (props) => {
  ...
  const data = _.chunk(props?.data, 4)
        ...
        <Slider {...settings} className='slide my-4'>
          {data?.map((item, i) => {
            const newData = item.map((data, index) => {
              return (
                <div
                  ...
                  data-bs-target='#SliderVideoListModal'
                  onClick={() => listenerIframe(data?.video)}
                >
                  {data?.source && (
                    <img className='image w-100' src={data?.source} alt='' />
                  )}
            ...
            return (
              <div
                key={i.toString()}
                className='SliderVideoList-data d-flex flex-wrap'
              >
                {newData}
              </div>
            )
