            ...
            return (
                <div className="row my-4">
                    {dataAPI?.data?.map((item, i) => {
                        const {title, body, field_code_couleur_accueil, field_lien_accueil} = item?.attributes
                        // console.log(title)

                        return (
                            <div key={i.toString()} className="col-md-3">
                                <Cards
                                    className="CardCommanderieCroyants m-auto d-flex justify-content-right align-items-right flex-column">
                                    {/* <img className="img-responsive my-1" src={`${base_url}/${dataAPI?.included[i]?.attributes?.uri?.url}`} alt={title} /> */}
                                    <h5 className="title mt-3"
                                        style={{color: field_code_couleur_accueil ? `#${field_code_couleur_accueil}` : "#129d59"}}>{title}</h5>
                                    {/* <p className="description pt-3 pb-2">{item.description}</p> */}
                                    {<div className="description pt-3 pb-2"
                                          dangerouslySetInnerHTML={{__html: body?.processed}}/>}
                                    <Link
                                        role="button"
                                        //to={field_lien[0]?.uri.slice(9)}
                                        href={{
                                            pathname: field_lien_accueil ? field_lien_accueil[0]?.uri.slice(9) : "",
                                            search: '',
                                            hash: '',
